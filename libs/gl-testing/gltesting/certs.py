...
def genca(idpath):
    ...
    cert_sign_json = subprocess.check_output(["cfssl", "sign", f"-ca={parent[0]}", f"-ca-key={parent[1]}", f"-config={tmpconfig.name}", f"-profile={profile}", path[3] + ".csr"])
    subprocess.run(["cfssljson", "-bare", path[3]], input=cert_sign_json)
...
def gencert_from_csr(csr: bytes, recover=False, pairing=False):
    ...
    if pairing:
        certs_json = subprocess.check_output(["cfssl", "sign", f"-ca={parent[0]}", f"-ca-key={parent[1]}", tmpcsr.name, tmpsubject.name])
        subprocess.run(["cfssljson", "-bare", path[3]], input=certs_json)
    else:
        certs_json = subprocess.check_output(["cfssl", "sign", f"-ca={parent[0]}", f"-ca-key={parent[1]}", tmpcsr.name])
        subprocess.run(["cfssljson", "-bare", path[3]], input=certs_json)
